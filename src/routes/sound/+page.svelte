<script lang="ts">
	...
	let embeds = $state({
		sugarcane: { status: 'loading' as EmbedStatus, timeoutId: undefined as number | undefined },
		orbitalFarm: { status: 'loading' as EmbedStatus, timeoutId: undefined as number | undefined }
	});

	const LOAD_TIMEOUT_MS = 15000; // 15 seconds

	function handleIframeLoad(embedKey: 'sugarcane' | 'orbitalFarm') {
		if (embeds[embedKey].timeoutId) {
			clearTimeout(embeds[embedKey].timeoutId);
		}
		embeds[embedKey].status = 'loaded';
	}

	function handleIframeError(embedKey: 'sugarcane' | 'orbitalFarm') {
		if (embeds[embedKey].timeoutId) {
			clearTimeout(embeds[embedKey].timeoutId);
		}
		embeds[embedKey].status = 'error';
	}

	function setupTimeout(embedKey: 'sugarcane' | 'orbitalFarm') {
		const timeoutId = setTimeout(() => {
			if (embeds[embedKey].status === 'loading') {
				embeds[embedKey].status = 'error';
			}
		}, LOAD_TIMEOUT_MS) as unknown as number;

		embeds[embedKey].timeoutId = timeoutId;
	}

	function retryLoad(embedKey: 'sugarcane' | 'orbitalFarm') {
		embeds[embedKey].status = 'loading';
		setupTimeout(embedKey);
	}
